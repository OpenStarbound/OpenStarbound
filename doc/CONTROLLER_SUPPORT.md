...
**Left stick (movement):** Always active regardless of input mode (except Off). Uses deadzone normalization — values below the deadzone are ignored, values above are remapped to 0.0–1.0 range. This feeds into `Player::setMoveVector()` which provides proportional walk/run speed. Additionally, when the Y axis exceeds a configurable threshold (`controllerVerticalThreshold`, default 0.65), `moveUp()`/`moveDown()` are called for zero-gravity flight, swimming, ladder climbing, and platform drop-through (down + jump).
...
Several panes require right-click (navigation console for travel, inventory for stack splitting). In Hybrid mode, trackpad click is left-click only. R3 (right stick click) generates a synthetic right-click when any window/modal pane is open. When no pane is open, R3 is camera pan (existing CameraShift behavior). Camera pan is suppressed when R3 is used as right-click in a pane to prevent both actions firing simultaneously.

### Cursor Crosshair Suppression

When the controller handles aim (Hybrid, Gamepad, or Auto with gamepad active), weapon crosshair cursors are suppressed — the mouse cursor shows Starbound's default cursor instead. Contextual cursor overrides (inspection tool, pane interaction highlights) still render normally. This prevents the misleading appearance of a crosshair following the mouse when the mouse isn't controlling aim.
...
### Shift Toggle vs Per-Frame Override

`Player::setShifting()` is called every frame from the main game loop based on whether the shift key is held (`isActionTaken(InterfaceAction::PlayerShifting)`). A one-shot toggle (calling `setShifting(!shifting())`) gets immediately overwritten next frame. The controller MM single-block mode uses a persistent `m_controllerShiftToggle` bool that's OR'd with the keyboard shift state each frame. The toggle resets when switching away from the matter manipulator.

### Duplicate JSON Keys Crash

Starbound's JSON parser rejects duplicate keys in the same object. During the consolidation of binds into `controller.binds`, a `CameraShift` entry existed in both the interface group and the context group, causing a fatal parse error on startup. JSON validity checkers (like Python's `json.load`) don't catch this since the JSON spec is ambiguous on duplicate keys.
...
| File | Purpose |
|------|---------|
| `assets/opensb/binds/controller.binds` | All controller binds: game actions, virtual cursor, context actions |
...
- **Double-duty binds (L3, R3) aren't rebindable as separate functions.** L3 is virtual cursor toggle in Gamepad mode but shift/drop in Hybrid mode — this context-dependent behavior is hardcoded, not split into separate rebindable binds. However, a standalone "Shift / Drop" bind is available (unbound by default) under Controller → Context Actions in Mod Binds for users who want this on a dedicated button in any mode.

...
char* ConvertShaderBuiltInVariableOnly(const char* pEntry, int isVertex, shaderconv_need_t* need,
    ...
    int headline = 3;
    int notexarray = globals4es.notexarray || need->need_notexarray;
    ...
    if (isVertex) {
        ...
        if (doInsertDefinitions && strstr(Tmp, "ftransform(")) {
        ...
        for (int i = 0; i < n; i++) {
            if (strstr(Tmp, builtin_attrib[i].glname)) {
                ...
                if (doInsertDefinitions) {
                    char def[100];
                    sprintf(def, "attribute %s %s %s;\n", builtin_attrib[i].prec, builtin_attrib[i].type,
    ...
    if (strstr(Tmp, "gl_Color") || need->need_color) {
        ...
        Tmp = InplaceReplace(Tmp, &tmpsize, "gl_Color",
                             (need->need_color == 1) ? "gl_FrontColor" : "(gl_FrontFacing?gl_FrontColor:gl_BackColor)");
    }
    ...
    // builtin matrices work
    {
        if (strstr(Tmp, "transpose(") || strstr(Tmp, "transpose ") || strstr(Tmp, "transpose\t")) {
            if (doInsertDefinitions) Tmp = InplaceInsert(GetLine(Tmp, headline), gl4es_transpose, Tmp, &tmpsize);
    ...
    // Handling of gl_LightSource[x].halfVector => normalize(gl_LightSource[x].position - gl_Vertex), but what if in the
    // FragShader ?
    ...
    if (strstr(Tmp, "gl_LightModelProducts") || strstr(Tmp, "gl_FrontLightModelProduct") ||
        strstr(Tmp, "gl_BackLightModelProduct")) {
    ...
    if (strstr(Tmp, "gl_LightModel")) Tmp = InplaceReplace(Tmp, &tmpsize, "gl_LightModel", "_gl4es_LightModel");
    ...
    if (strstr(Tmp, "gl_BackMaterial")) Tmp = InplaceReplace(Tmp, &tmpsize, "gl_BackMaterial", "_gl4es_BackMaterial");
    ...
    if (strstr(Tmp, "gl_Point")) Tmp = InplaceReplace(Tmp, &tmpsize, "gl_Point", "_gl4es_Point");
    if (strstr(Tmp, "gl_FogParameters") || strstr(Tmp, "gl_Fog")) {
        Tmp = InplaceInsert(GetLine(Tmp, headline),
                            hardext.highp ? gl4es_FogParametersSourceHighp : gl4es_FogParametersSource, Tmp, &tmpsize);
        headline += CountLine(gl4es_FogParametersSource);
        Tmp = InplaceReplace(Tmp, &tmpsize, "gl_FogParameters", "_gl4es_FogParameters");
    }
    if (strstr(Tmp, "gl_Fog")) Tmp = InplaceReplace(Tmp, &tmpsize, "gl_Fog", "_gl4es_Fog");
    ...
    if (FindString(Tmp, gl_ProgramEnv)) {
        ...
        if (noarray_ok) {
            ...
            for (int i = 0; i <= maxind; ++i) {
                ...
                if (doInsertDefinitions && FindString(Tmp, T)) {
...
#undef GO
    ...
    if (versionHeader > 1) {
        ...
        if (doInsertDefinitions) {
            if (isVertex)
                GLESBackport = "#define attribute in\n#define varying out\n";
            else
                GLESBackport = "#define varying in\n";
...
char* ConvertShader(const char* pEntry, int isVertex, shaderconv_need_t* need, int forwardPort) {
    ...
    if (version120) {
        if (hardext.glsl120)
            versionHeader = 1;
        else if (hardext.glsl320es)
            versionHeader = 4;
        else if (hardext.glsl310es)
            versionHeader = 2;
        else if (hardext.glsl300es) {
            versionHeader = 3; /* location on uniform not supported ! */
        }
        /* else no location or in / out are supported */
    }
    // sprintf(GLESFullHeader, GLESHeader, (wanthighp && hardext.highp==1 && !isVertex)?GLESUseFragHighp:"",
    // (wanthighp)?"highp":"mediump", (wanthighp)?"highp":"mediump");
    sprintf(GLESFullHeader, GLESHeader[versionHeader], "", (wanthighp) ? "highp" : "mediump",
    ...
    if (threed_texture) {
        Tmp = InplaceInsert(GetLine(Tmp, 1), GLESUse3DTexture, Tmp,
                            &tmpsize); // no fallback possible if device does not support 3D textures
    }
    ...
    if (!isVertex && hardext.shaderlod &&
        (FindString(Tmp, "texture2DLod") || FindString(Tmp, "texture2DProjLod") || FindString(Tmp, "textureCubeLod") ||
         FindString(Tmp, "texture2DGradARB") || FindString(Tmp, "texture2DProjGradARB") ||
         FindString(Tmp, "textureCubeGradARB"))) {
    ...
    if (!isVertex && (FindString(Tmp, "textureCubeLod"))) {
        if (hardext.shaderlod) {
            if (!hardext.cubelod) Tmp = InplaceReplace(Tmp, &tmpsize, "textureCubeLod", "textureCubeLodEXT");
        } else {
    ...
    if (!isVertex && (FindString(Tmp, "textureCubeGradARB"))) {
        if (hardext.shaderlod) {
            if (!hardext.cubelod) Tmp = InplaceReplace(Tmp, &tmpsize, "textureCubeGradARB", "textureCubeGradEXT");
        } else {
            ...
            for (int i = 0; i < n; i++) {
                if (strstr(Tmp, builtin_matrix[i].glname)) {
                    ...
                    if (builtin_matrix[i].texarray)
                        ...
                        sprintf(def, "uniform %s%s %s;\n", (ishighp) ? "highp " : "mediump ", builtin_matrix[i].type,
    ...
    // Handling of gl_LightSource[x].halfVector => normalize(gl_LightSource[x].position - gl_Vertex), but what if in
    // the FragShader ?
    /*  if(strstr(Tmp, "halfVector"))
      {
        char *p = Tmp;
...
int isBuiltinAttrib(const char* name) {
    int n = sizeof(builtin_attrib) / sizeof(builtin_attrib_t);
    for (int i = 0; i < n; i++) {
        if (strcmp(builtin_attrib[i].name, name) == 0) return builtin_attrib[i].attrib;
    }
    return -1;
}
    ...
    for (int i = 0; i < n && ret == -1; i++) {
        if (strncmp(builtin_matrix[i].name, name, strlen(builtin_matrix[i].name)) == 0) {
            ...
            if (strlen(name) == l || (strlen(name) == l + 3 && name[l] == '[' && builtin_matrix[i].texarray) ||
                ...
                if (builtin_matrix[i].texarray) {
                    int n = name[l + 1] - '0';
                    if (name[l + 2] >= '0' && name[l + 2] <= '9') n = n * 10 + name[l + 2] - '0';
                    ret += n * 4;
                }
            }
        }
...
const char* hasBuiltinAttrib(const char* vertexShader, int Att) {
    ...
    if (hardext.maxvattrib > 8) {
        ...
        for (int i = 0; i < n && !ret; i++) {
            if (builtin_attrib[i].attrib == Att) ret = builtin_attrib[i].name;
        }
    } else {
        int n = sizeof(builtin_attrib_compressed) / sizeof(builtin_attrib_t);
        for (int i = 0; i < n && !ret; i++) {
            if (builtin_attrib_compressed[i].attrib == Att) ret = builtin_attrib_compressed[i].name;
        }
    }
    if (!ret) return NULL;
...
const char* builtinAttribGLName(const char* name) {
    ...
    int n = sizeof(builtin_attrib) / sizeof(builtin_attrib_t);
    for (int i = 0; i < n; ++i)
        if (!strcmp(name, builtin_attrib[i].name)) return builtin_attrib[i].glname;
...
const char* builtinAttribInternalName(const char* name) {
    ...
    int n = sizeof(builtin_attrib) / sizeof(builtin_attrib_t);
    for (int i = 0; i < n; ++i)
        if (!strcmp(name, builtin_attrib[i].glname)) return builtin_attrib[i].name;

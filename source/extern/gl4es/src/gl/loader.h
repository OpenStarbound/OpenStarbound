...
#include <EGL/eglext.h>
...
#ifdef TEXSTREAM
    typedef EGLSurface (*eglCreatePixmapSurfaceHI_PTR)(EGLDisplay dpy, EGLConfig config,
    ...
    typedef EGLImageKHR (*eglCreateImageKHR_PTR)(EGLDisplay dpy, EGLContext ctx, EGLenum target, EGLClientBuffer buffer,
                                                 const EGLint* attrib_list);
    typedef EGLStreamKHR (*eglCreateStreamFromFileDescriptorKHR_PTR)(EGLDisplay dpy,
    ...
    typedef EGLBoolean (*eglQueryStreamTimeKHR_PTR)(EGLDisplay dpy, EGLStreamKHR stream, EGLenum attribute,
                                                    EGLTimeKHR* value);
    typedef EGLint (*eglWaitSyncKHR_PTR)(EGLDisplay dpy, EGLSyncKHR sync, EGLint flags);
    typedef EGLSurface (*eglCreateStreamProducerSurfaceKHR_PTR)(EGLDisplay dpy, EGLConfig config, EGLStreamKHR stream,
                                                                const EGLint* attrib_list);
#endif // TEXSTREAM
#endif // NOEGL
...
#include <dlfcn.h>
    ...
    __declspec(dllimport) FPROC __stdcall GetProcAddress(HISTANCE, const char*);
#endif
#ifdef _MSC_VER
    __forceinline
...
#define WARN_NULL(name)                                                                                                \
    if (name == NULL) LOGD("warning, %s line %d function %s: " #name " is NULL\n", __FILE__, __LINE__, __func__);
    ...
    {                                                                                                                  \
        ...
        if (first) {                                                                                                   \
            ...
            if (lib != NULL) {                                                                                         \
                ...
                if (!lib##_##name) lib##_##name = (name##_PTR)fnc2;                                                    \
                if (!lib##_##name) {                                                                                   \
                    __VA_ARGS__                                                                                        \
                }                                                                                                      \
            }                                                                                                          \
...
#define LOAD_RAW_ALT(lib, alt, name, ...)                                                                              \
    {                                                                                                                  \
        ...
        if (first) {                                                                                                   \
            ...
            if (lib##_##name == NULL) lib##_##name = alt##_##name;                                                     \
        }                                                                                                              \
    }
...
#define LOAD_EGL_EXT(name)                                                                                             \
    DEFINE_RAW(egl, name);                                                                                             \
    LOAD_RAW_3(egl, name, proc_address(egl, #name), proc_address(egl, #name "EXT"), LOAD_EGL(eglGetProcAddress);       \
               LOAD_RAW(egl, name, egl_eglGetProcAddress(#name "EXT"));)

...
#define DBG(a)
    ...
    GLenum APIENTRY_GL4ES gl4es_glGetError(void) {
        ...
        if (!glstate->type_error) {
            ...
            if (err == GL_NO_ERROR) err = glstate->shim_error;
        } else {
            err = glstate->shim_error;
        }
        if (glstate->type_error == 1) gles_glGetError(); // purge error log
    ...
    void BuildExtensionsList() {
        if (!glstate->extensions) {
            ...
            strcpy(extensions,
                   "GL_EXT_abgr "
#ifdef AMIGAOS4
                   "GL_MGL_packed_pixels " // same as GL_EXT_packed_pixels, but older, some old Amiga games may check
                                           // for this
#endif
                   "GL_EXT_packed_pixels "
                   "GL_EXT_compiled_vertex_array "
                   "GL_EXT_compiled_vertex_arrays " // yes, at least on AmigaOS there are progs which check for this
                                                    // wrong string; reason is an old typo in the MiniGL driver.
            ...
            if (globals4es.npot >= 2) strcat(extensions, "GL_ARB_texture_non_power_of_two ");
            if (hardext.blendcolor) strcat(extensions, "GL_EXT_blend_color ");
            ...
            if (hardext.pointsprite) strcat(extensions, "GL_ARB_point_sprite ");
            if (hardext.cubemap) {
            ...
            if (hardext.esversion > 1) {
                ...
                strcat(extensions, "GL_ARB_fragment_shader "
                ...
                if (!globals4es.noarbprogram)
                    strcat(extensions, "GL_ARB_vertex_program "
    ...
    int gl4es_commonGet(GLenum pname, GLfloat* params) {
        ...
        case GL_TEXTURE_BINDING_1D:
            *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX1D]
                          ? glstate->texture.bound[glstate->texture.active][ENABLED_TEX1D]->glname
                          : 0;
            break;
        case GL_TEXTURE_BINDING_2D:
            *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D]
                          ? glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D]->glname
                          : 0;
            break;
        case GL_TEXTURE_BINDING_3D:
            *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX3D]
                          ? glstate->texture.bound[glstate->texture.active][ENABLED_TEX3D]->glname
                          : 0;
            break;
...
#undef GO
    ...
    void APIENTRY_GL4ES gl4es_glGetMaterialfv(GLenum face, GLenum pname, GLfloat* params) {
        ...
        switch (pname) {
        case GL_AMBIENT:
            if (face == GL_FRONT) memcpy(params, glstate->material.front.ambient, 4 * sizeof(GLfloat));
            if (face == GL_BACK) memcpy(params, glstate->material.back.ambient, 4 * sizeof(GLfloat));
            break;
        case GL_DIFFUSE:
            if (face == GL_FRONT) memcpy(params, glstate->material.front.diffuse, 4 * sizeof(GLfloat));
            if (face == GL_BACK) memcpy(params, glstate->material.back.diffuse, 4 * sizeof(GLfloat));
            break;
        case GL_SPECULAR:
            if (face == GL_FRONT) memcpy(params, glstate->material.front.specular, 4 * sizeof(GLfloat));
            if (face == GL_BACK) memcpy(params, glstate->material.back.specular, 4 * sizeof(GLfloat));
            break;
        case GL_EMISSION:
            if (face == GL_FRONT) memcpy(params, glstate->material.front.emission, 4 * sizeof(GLfloat));
            if (face == GL_BACK) memcpy(params, glstate->material.back.emission, 4 * sizeof(GLfloat));
            break;
        case GL_SHININESS:
            if (face == GL_FRONT) *params = glstate->material.front.shininess;
            if (face == GL_BACK) *params = glstate->material.back.shininess;
            break;

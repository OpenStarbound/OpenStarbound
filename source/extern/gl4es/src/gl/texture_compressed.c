...
GLvoid* uncompressDXTc(GLsizei width, GLsizei height, GLenum format, GLsizei imageSize, int transparent0,
                       int* simpleAlpha, int* complexAlpha, const GLvoid* data) {
...
void APIENTRY_GL4ES gl4es_glCompressedTexImage2D(GLenum target, GLint level, GLenum internalformat, GLsizei width,
    ...
    DBG(SHUT_LOGD("glCompressedTexImage2D on target=%s:%p, level=%d with size(%i,%i), internalformat=%s, imagesize=%i, "
                  "upackbuffer=%p data=%p\n",
                  PrintEnum(target), bound, level, width, height, PrintEnum(internalformat), imageSize,
                  glstate->vao->unpack ? glstate->vao->unpack->data : 0, data);)
    // hack...
    if (internalformat == GL_RGBA8) internalformat = GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
    ...
    if (level && (GL4ES_AUTOMIPMAP_PLACEHOLDER == 3)) {
        noerrorShim();
    ...
    if (isDXTc(internalformat)) {
        if (level && bound->mipmap_auto == 1) return; // nothing to do
        GLvoid *pixels, *half;
        pixels = half = NULL;
        bound->alpha =
            (internalformat == GL_COMPRESSED_RGB_S3TC_DXT1_EXT || internalformat == GL_COMPRESSED_SRGB_S3TC_DXT1_EXT)
                ? 0
                : 1;
            ...
            if (globals4es.avoid16bits) {
                ...
                format = (internalformat == GL_COMPRESSED_RGB_S3TC_DXT1_EXT ||
                          internalformat == GL_COMPRESSED_SRGB_S3TC_DXT1_EXT)
                             ? GL_RGB
                             : GL_RGBA;
                type = (internalformat == GL_COMPRESSED_RGB_S3TC_DXT1_EXT ||
                        internalformat == GL_COMPRESSED_SRGB_S3TC_DXT1_EXT)
                           ? GL_UNSIGNED_SHORT_5_6_5
                           : ((internalformat == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) ? GL_UNSIGNED_SHORT_5_5_5_1
        ...
        int simpleAlpha = 0;
        int complexAlpha = 0;
        int transparent0 = (internalformat == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ||
        ...
        if (datab) {
            if ((width & 3) || (height & 3)) { // can happens :(
                ...
                if (nh < 4) nh = 4;
                tmp =
                    uncompressDXTc(nw, nh, internalformat, imageSize, transparent0, &simpleAlpha, &complexAlpha, datab);
                ...
                pixels = uncompressDXTc(width, height, internalformat, imageSize, transparent0, &simpleAlpha,
                                        &complexAlpha, datab);
            }
        ...
        DBG(SHUT_LOGD(" => internalformat=%s (Alpha=%d/%d), %dx%d %s/%s\n\n", PrintEnum(new_intformat), simpleAlpha,
                      complexAlpha, width, height, PrintEnum(format), PrintEnum(type));)
...
void APIENTRY_GL4ES gl4es_glGetCompressedTexImage(GLenum target, GLint lod, GLvoid* img) {
    ...
    gltexture_t* bound = glstate->texture.bound[glstate->texture.active][itarget];
    DBG(SHUT_LOGD(
            "glGetCompressedTexImage(%s, %i, %p), bound=%p, size=%dx%d, bound->orig_internal=%s/wanted_internal=%s\n",
            PrintEnum(target), lod, img, bound, bound ? bound->width : 0, bound ? bound->height : 0,
            (bound) ? PrintEnum(bound->orig_internal) : "nil", (bound) ? PrintEnum(bound->wanted_internal) : "nil");)
    errorShim(GL_INVALID_OPERATION);
    if (bound->orig_internal != GL_COMPRESSED_RGB && bound->orig_internal != GL_COMPRESSED_RGBA) return;
    int width = nlevel(bound->width, lod);
    int height = nlevel(bound->height, lod);
    int w = nlevel(width, 2);
    w <<= 2;
    int h = nlevel(height, 2);
    h <<= 2;

    int alpha = (bound->orig_internal == GL_COMPRESSED_RGBA) ? 1 : 0;
    int dxt1 = (bound->wanted_internal == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ||
                bound->wanted_internal == GL_COMPRESSED_RGB_S3TC_DXT1_EXT)
                   ? 1
                   : 0; // Add SRGB variant?
    ...
    for (int y = 0; y < h; y += 4)
        for (int x = 0; x < w; x += 4) {
            GLuint col = 0;
            for (int i = 0; i < 16; i++) {
                if (x + (i % 4) < width && y + (i / 4) < height) col = src[x + (i % 4) + (y + (i / 4)) * width];
                tmp[i] = col;
                if (alpha && dxt1) {
                    // change transparent to RGB = 0
                    for (int i = 0; i < 16; ++i)
                        if ((tmp[i] & 0xff000000) != 0xff000000) tmp[i] = 0;
                }
            }
...
// Direct wrapper
AliasExport(void, glCompressedTexImage2D, ,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLint border,
             GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexImage1D, ,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLint border, GLsizei imageSize,
             const GLvoid* data));
AliasExport(void, glCompressedTexImage3D, ,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLsizei depth,
             GLint border, GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexSubImage2D, ,
            (GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format,
             GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexSubImage1D, ,
            (GLenum target, GLint level, GLint xoffset, GLsizei width, GLenum format, GLsizei imageSize,
             const GLvoid* data));
AliasExport(void, glCompressedTexSubImage3D, ,
...
// EXT mapper
AliasExport(void, glCompressedTexImage2D, EXT,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLint border,
             GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexImage1D, EXT,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLint border, GLsizei imageSize,
             const GLvoid* data));
AliasExport(void, glCompressedTexImage3D, EXT,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLsizei depth,
             GLint border, GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexSubImage2D, EXT,
            (GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format,
             GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexSubImage1D, EXT,
            (GLenum target, GLint level, GLint xoffset, GLsizei width, GLenum format, GLsizei imageSize,
             const GLvoid* data));
AliasExport(void, glCompressedTexSubImage3D, EXT,
            (GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height,
             GLsizei depth, GLenum format, GLsizei imageSize, const GLvoid* data));
AliasExport(void, glGetCompressedTexImage, EXT, (GLenum target, GLint lod, GLvoid* img));

// ARB mapper
AliasExport(void, glCompressedTexImage2D, ARB,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLint border,
             GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexImage1D, ARB,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLint border, GLsizei imageSize,
             const GLvoid* data));
AliasExport(void, glCompressedTexImage3D, ARB,
            (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLsizei depth,
             GLint border, GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexSubImage2D, ARB,
            (GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format,
             GLsizei imageSize, const GLvoid* data));
AliasExport(void, glCompressedTexSubImage1D, ARB,
            (GLenum target, GLint level, GLint xoffset, GLsizei width, GLenum format, GLsizei imageSize,
             const GLvoid* data));
AliasExport(void, glCompressedTexSubImage3D, ARB,
            (GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height,
             GLsizei depth, GLenum format, GLsizei imageSize, const GLvoid* data));

...
glframebuffer_t* get_framebuffer(GLenum target) {
    ...
    case GL_READ_FRAMEBUFFER:
        return glstate->fbo.fbo_read;
    case GL_DRAW_FRAMEBUFFER:
        return glstate->fbo.fbo_draw;
    }
    return NULL;
}

void readfboBegin() {
    if (glstate->fbo.fbo_read == glstate->fbo.fbo_draw) return;
    DBG(SHUT_LOGD("readfboBegin, fbo status read=%u, draw=%u, main=%u, current=%u\n", glstate->fbo.fbo_read->id,
                  glstate->fbo.fbo_draw->id, glstate->fbo.mainfbo_fbo, glstate->fbo.current_fb->id);)
    if (glstate->fbo.fbo_read == glstate->fbo.current_fb) return;
...
void readfboEnd() {
    if (glstate->fbo.fbo_read->id == glstate->fbo.fbo_draw->id) return;
    DBG(SHUT_LOGD("readfboEnd, fbo status read=%p, draw=%p, main=%u, current=%p\n", glstate->fbo.fbo_read,
                  glstate->fbo.fbo_draw, glstate->fbo.mainfbo_fbo, glstate->fbo.current_fb);)
    if (glstate->fbo.fbo_draw == glstate->fbo.current_fb) return;
...
GLenum ReadDraw_Push(GLenum target) {
    ...
    //                                (glstate->fbo.fbo_draw->id) ? glstate->fbo.fbo_draw->id :
    //                                glstate->fbo.mainfbo_fbo);
    //     return GL_DRAW_FRAMEBUFFER;
    // }
    // if (target == GL_READ_FRAMEBUFFER) {
    //     if (glstate->fbo.current_fb != glstate->fbo.fbo_read)
    //         gles_glBindFramebuffer(GL_READ_FRAMEBUFFER,
    //                                (glstate->fbo.fbo_read->id) ? glstate->fbo.fbo_read->id :
    //                                glstate->fbo.mainfbo_fbo);
...
void APIENTRY_GL4ES gl4es_glFramebufferTexture2D(GLenum target, GLenum attachment, GLenum textarget, GLuint texture,
    ...
    if (target == GL_FRAMEBUFFER) {
        gl4es_glFramebufferTexture2D(GL_FRAMEBUFFER - 1, attachment, textarget, texture, level);
        gl4es_glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, attachment, textarget, texture, level);
        gl4es_glFramebufferTexture2D(GL_READ_FRAMEBUFFER, attachment, textarget, texture, level);
        return;
    }
    ...
    if (texture) {
        ...
        if (!tex) {
            ...
            if (globals4es.fbtexscale > 0.0f || tex->shrink || tex->useratio ||
                (tex->adjust && (hardext.npot == 1 || hardext.npot == 2) && !globals4es.potframebuffer)) {
            ...
            int need_change =
                (globals4es.potframebuffer && (npot(twidth) != twidth || npot(theight) != theight)) ? 1 : 0;
            if ((tex->type == GL_FLOAT && !hardext.floatfbo) ||
                (tex->type == GL_HALF_FLOAT_OES && !hardext.halffloatfbo)) {
                need_change += 2;
            ...
            if (need_change) {
                ...
                LOGD("Recreate a texture for a FBO (%s%s%s)\n", (need_change & 1) ? "POT" : "",
                     (need_change == 3) ? " & " : "", (need_change & 2) ? "Format/Type" : "");
                if (need_change & 1) {
            ...
            /*           if ((tex->width<32) || (tex->height<32)) {
                           SHUT_LOGD("LIBGL: enlarging too-small texture for FBO\n");
               ...
               tex->type, NULL); if (oldtex!=tex->glname) gles_glBindTexture(GL_TEXTURE_2D, oldtex);
                       }*/

            DBG(SHUT_LOGD("found texture, glname=%u, size=%ix%i(%ix%i), format/type=%s/%s\n", texture, tex->width,
                          tex->height, tex->nwidth, tex->nheight, PrintEnum(tex->format), PrintEnum(tex->type));)
        }
    }

    int level_width = twidth;
    int level_height = theight;
    if (level_width > 0) {
        level_width >>= level;
    ...
    fb->width = level_width;
    fb->height = level_height;
    ...
    if (old_attachment) {
        ...
        if (old) {
            DBG(SHUT_LOGD("Detach Texture %d from FBO %d as Attachement %s\n", old->glname, old->binded_fbo,
                          PrintEnum(old->binded_attachment));)
            old->binded_fbo = 0;
            old->binded_attachment = 0;
        } else {
            DBG(SHUT_LOGD("Old attachement %s for FBO %d not found in textures\n", PrintEnum(attachment), fb->id);)
        }
    }

    if (tex) {
        DBG(SHUT_LOGD("Attach Texture %d to FBO %d as Attachement %s\n", tex->glname, fb->id, PrintEnum(attachment));)
    ...
    if (attachment >= GL_COLOR_ATTACHMENT0 && attachment < (GL_COLOR_ATTACHMENT0 + hardext.maxcolorattach) && tex) {
        gltexture_t* bound = glstate->texture.bound[0 /*glstate->texture.active*/][ENABLED_TEX2D];
        if ((hardext.npot == 1 || hardext.npot == 2) &&
            (!tex->actual.wrap_s || !tex->actual.wrap_t || !wrap_npot(tex->actual.wrap_s) ||
             !wrap_npot(tex->actual.wrap_t))) {
            tex->sampler.wrap_s = tex->sampler.wrap_t = GL_CLAMP_TO_EDGE;
            tex->adjust = 0;
        }
        // npot==2 and 3 should support that, but let's ignore that for now and force no mipmap for texture attached to
        // fbo...
        if (!tex->actual.min_filter || !minmag_npot(tex->actual.min_filter) ||
            tex->actual.min_filter != tex->sampler.min_filter) {
            if (hardext.npot < 2) {
    ...
    if (attachment == GL_DEPTH_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        if (hardext.depthtex && (tex || !texture)) {
            ...
            gles_glFramebufferTexture2D(ntarget, attachment, GL_TEXTURE_2D, texture, level);
        } else {
            ...
            if (tex && !tex->renderdepth) {
                ...
                gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT32F, level_width, level_height);
    ...
    if (attachment == GL_STENCIL_ATTACHMENT /*&& hardext.depthtex==0*/) {
        noerrorShim();
        // this one is tricky, as it can be GL_DEPTH_STENCIL with DEPTH+DEPTH_STENCIL extension of GL_STENCIL_INDEX8
        // with STENCIL extension and having DEPTH+DEPTH_STENCIL extension doesn't grant STENCIL8 texture extension!
        if ((tex || !texture) && (hardext.stenciltex || (hardext.depthtex && hardext.depthstencil))) {
            ...
            if (tex && !(tex->format == GL_STENCIL_INDEX8 || tex->format == GL_DEPTH_STENCIL)) {
                if (tex->format == GL_DEPTH_COMPONENT) {
                    ...
                    if (tex->type != GL_UNSIGNED_BYTE && tex->type != GL_UNSIGNED_SHORT && tex->type != GL_FLOAT)
                        tex->type = GL_UNSIGNED_BYTE;
                    tex->fpe_format = FPE_TEX_DEPTH;
                    realize_textures(0);
                    int oldactive = glstate->texture.active;
            ...
            if (tex && !tex->renderstencil) {
                ...
                gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_STENCIL_INDEX8, level_width, level_height);
                gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
            }
    ...
    if (attachment == GL_DEPTH_STENCIL_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        if (hardext.depthstencil /*&& !(hardext.depthtex && tex)*/) {
            if (hardext.depthtex && (tex || !texture)) {
                ...
                gles_glFramebufferTexture2D(ntarget, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, texture, level);
                gles_glFramebufferTexture2D(ntarget, GL_STENCIL_ATTACHMENT, GL_TEXTURE_2D, texture, level);
            } else {
                if (tex && !tex->renderdepth) {
                    gl4es_glGenRenderbuffers(1, &tex->renderdepth);
                    gl4es_glBindRenderbuffer(GL_RENDERBUFFER, tex->renderdepth);
                    gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, level_width, level_height);
                    gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
                }
                errorGL();
                gl4es_glFramebufferRenderbuffer(ntarget, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER,
                                                tex ? tex->renderdepth : 0);
                gl4es_glFramebufferRenderbuffer(ntarget, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                                tex ? tex->renderdepth : 0);
            }
        } else {
    ...
    gles_glFramebufferTexture2D(ntarget, attachment, realtarget, texture, level);
    DBG(CheckGLError(1);)
    ReadDraw_Pop(target);
}

void APIENTRY_GL4ES gl4es_glFramebufferTexture1D(GLenum target, GLenum attachment, GLenum textarget, GLuint texture,
                                                 GLint level) {
    gl4es_glFramebufferTexture2D(target, attachment, textarget, texture, level);
}
void APIENTRY_GL4ES gl4es_glFramebufferTexture3D(GLenum target, GLenum attachment, GLenum textarget, GLuint texture,
...
void APIENTRY_GL4ES gl4es_glGenRenderbuffers(GLsizei n, GLuint* renderbuffers) {
    DBG(SHUT_LOGD("glGenRenderbuffers(%i, %p)\n", n, renderbuffers);)
...
void APIENTRY_GL4ES gl4es_glFramebufferRenderbuffer(GLenum target, GLenum attachment, GLenum renderbuffertarget,
    ...
    if (attachment == GL_STENCIL_ATTACHMENT) {
        if (rend && rend->secondarybuffer) renderbuffer = rend->secondarybuffer;
    }
...
void APIENTRY_GL4ES gl4es_glBindRenderbuffer(GLenum target, GLuint renderbuffer) {
    ...
    if (renderbuffer == 0) {
        if (glstate->fbo.current_rb == glstate->fbo.default_rb) {
            noerrorShim();
            return;
        }
        glstate->fbo.current_rb = glstate->fbo.default_rb;
        errorGL();
        gles_glBindRenderbuffer(target, 0);
        return;
    }
...
GLboolean APIENTRY_GL4ES gl4es_glIsRenderbuffer(GLuint renderbuffer) {
    DBG(SHUT_LOGD("glIsRenderbuffer(%u)\n", renderbuffer);)
    if (renderbuffer == 0) {
        noerrorShim();
        return GL_FALSE;
    }
...
void APIENTRY_GL4ES gl4es_glGetFramebufferAttachmentParameteriv(GLenum target, GLenum attachment, GLenum pname,
                                                                GLint* params) {
    ...
    */
    ...
    if (attachment == GL_DEPTH_ATTACHMENT && pname == GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE) {
        if (hardext.depthtex == 0) {
            ...
            *params = GetAttachment(fb, attachment);
            // TODO: Check all this?!
            if (*params) *params = 16; // Depth buffer is 16 on GLES. No check for 24 bits here...
            return;
        }
        // check if it's depth/stencil 16/8, in that case, lie by spoofing 24/8 or some FNA game will fail
        int depth, stencil;
        GLenum ntarget = ReadDraw_Push(target);
        gles_glGetFramebufferAttachmentParameteriv(ntarget, GL_STENCIL_ATTACHMENT,
                                                   GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE, &stencil);
        gles_glGetFramebufferAttachmentParameteriv(ntarget, GL_DEPTH_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE,
                                                   &depth);
        errorGL();
        ReadDraw_Pop(target);
        if (depth == 16 && stencil == 8) depth = 24;
        *params = depth;
        return;
    }
...
void APIENTRY_GL4ES gl4es_glGetRenderbufferParameteriv(GLenum target, GLenum pname, GLint* params) {
    DBG(SHUT_LOGD("glGetRenderbufferParameteriv(%s, %s, %p)\n", PrintEnum(target), PrintEnum(pname), params);)
...
void createMainFBO(int width, int height) {
    ...
    if (glstate->fbo.mainfbo_fbo) {
        if (width == glstate->fbo.mainfbo_width && height == glstate->fbo.mainfbo_height) return;
        // lets adjust the FBO instead of adjusting it
        createIt = 0;
    }
    DBG(SHUT_LOGD("LIBGL: Create FBO of %ix%i 32bits\n", width, height);)
    ...
    gles_glTexImage2D(GL_TEXTURE_2D, 0, globals4es.fbo_noalpha ? GL_RGB : GL_RGBA, width, height, 0,
                      globals4es.fbo_noalpha ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE, NULL);
...
void blitMainFBO(int x, int y, int width, int height) {
    if (glstate->fbo.mainfbo_fbo == 0) return;
    ...
    gl4es_blitTexture(glstate->fbo.mainfbo_tex, 0.f, 0.f, glstate->fbo.mainfbo_width, glstate->fbo.mainfbo_height,
                      glstate->fbo.mainfbo_nwidth, glstate->fbo.mainfbo_nheight, rx, ry, 0, 0, x, y, BLIT_OPAQUE);
...
void APIENTRY_GL4ES gl4es_glFramebufferTextureLayer(GLenum target, GLenum attachment, GLuint texture, GLint level,
                                                    GLint layer) {
    gl4es_glFramebufferTexture2D(target, attachment, GL_TEXTURE_2D, texture,
                                 level); // Force Texture2D, ignore layer (should track?)...
}
...
void APIENTRY_GL4ES gl4es_glBlitFramebuffer(GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0,
    ...
    // */
    // if (glstate->fbo.fbo_read == glstate->fbo.fbo_draw && srcX0 == dstX0 && srcX1 == dstX1 && srcY0 == dstY0 &&
    //     srcY1 == dstY1)
    //     return; // no need to try copying on itself
    //
    // if (dstX1 == dstX0 || dstY1 == dstY0) return; // nothing to draw
    // if (srcX1 == srcX0 || srcY1 == srcY0) return; // nothing to draw
    //
    // GLuint texture = (glstate->fbo.fbo_read->id == 0 && glstate->fbo.mainfbo_fbo) ? glstate->fbo.mainfbo_tex
    //                                                                               : glstate->fbo.fbo_read->color[0];
    //
    ...
    //         if ((glstate->fbo.mainfbo_width == abs(dstX1 - dstX0)) &&
    //             (glstate->fbo.mainfbo_height == abs(dstY1 - dstY0))) {
    //             blitfullscreen = 1;
    //         } else {
    ...
    // gl4es_blitTexture(glname, srcX0, srcY0, srcW, srcH, nwidth, nheight, zoomx, zoomy, 0, 0, dstX0, dstY0,
    // BLIT_OPAQUE); gl4es_glViewport(vp[0], vp[1], vp[2], vp[3]); if (created) {
...
static inline GLboolean is_default_fbo(const glframebuffer_t* fb) {
    return (!fb || fb->id == 0);
}

static inline GLenum remap_tex_target(GLenum textarget) {
    if (textarget >= GL_TEXTURE_CUBE_MAP_POSITIVE_X && textarget < GL_TEXTURE_CUBE_MAP_POSITIVE_X + 6) return textarget;
    return GL_TEXTURE_2D;
}

static void remap_drawbuffers_for_fbo(glframebuffer_t* fb, GLsizei n, const GLenum* bufs, GLenum* out_bufs,
                                      GLboolean rebind) {
    if (!bufs || !out_bufs || n <= 0) return;
    if (is_default_fbo(fb)) {
        memcpy(out_bufs, bufs, n * sizeof(GLenum));
        return;
    }
    LOAD_GLES2_OR_OES(glFramebufferTexture2D);
    LOAD_GLES2_OR_OES(glFramebufferRenderbuffer);

    GLenum attach_target = (hardext.esversion >= 3) ? GL_DRAW_FRAMEBUFFER : GL_FRAMEBUFFER;
    for (int i = 0; i < n; ++i) {
        GLenum buf = bufs[i];
        if (buf == GL_NONE) {
            out_bufs[i] = GL_NONE;
        ...
        if (buf >= GL_COLOR_ATTACHMENT0 && buf < (GL_COLOR_ATTACHMENT0 + hardext.maxcolorattach)) {
            int idx = buf - GL_COLOR_ATTACHMENT0;
            if (idx >= 0 && idx < MAX_DRAW_BUFFERS) {
                if (rebind) {
                    GLenum atttarget = fb->t_color[idx];
                    GLuint att = fb->color[idx];
                    int level = fb->l_color[idx];
                    if (atttarget == GL_RENDERBUFFER) {
                        gles_glFramebufferRenderbuffer(attach_target, GL_COLOR_ATTACHMENT0 + i, GL_RENDERBUFFER, att);
                    } else {
                        GLuint glname = 0;
                        GLenum realtarget = GL_TEXTURE_2D;
                        if (atttarget) {
                            realtarget = remap_tex_target(atttarget);
                            if (att) {
                                gltexture_t* tex = gl4es_getTexture(atttarget, att);
                                if (tex) glname = tex->glname;
                            }
                        }
                        gles_glFramebufferTexture2D(attach_target, GL_COLOR_ATTACHMENT0 + i, realtarget, glname, level);
                    }
                }
                out_bufs[i] = GL_COLOR_ATTACHMENT0 + i;
                continue;
            }
        }
        out_bufs[i] = buf;
...
void APIENTRY_GL4ES gl4es_glDrawBuffer(GLenum buffer) {
    ...
    glframebuffer_t* fb = glstate->fbo.fbo_draw;
    if (is_default_fbo(fb)) {
        ...
        glstate->fbo.fbo_draw->n_draw = 1;
        glstate->fbo.fbo_draw->drawbuff[0] = buffers[0];
    } else {
        if (buffer == GL_NONE ||
            (buffer >= GL_COLOR_ATTACHMENT0 && buffer < GL_COLOR_ATTACHMENT0 + hardext.maxcolorattach)) {
            gl4es_glDrawBuffers(1, &buffer);
        }
    }
}
...
void APIENTRY_GL4ES gl4es_glDrawBuffers(GLsizei n, const GLenum* bufs) {
    ...
    if (hardext.drawbuffers) {
        ...
        glframebuffer_t* fb = glstate->fbo.fbo_draw;
        if (!is_default_fbo(fb) && n > 0 && bufs) {
            GLenum* new_bufs = (GLenum*)alloca(n * sizeof(GLenum));
            remap_drawbuffers_for_fbo(fb, n, bufs, new_bufs, GL_TRUE);
...
void APIENTRY_GL4ES gl4es_glClearBufferiv(GLenum buffer, GLint drawbuffer, const GLint* value) {
    ...
    switch (buffer) {
    case GL_COLOR:
        if (drawbuffer < 0 || drawbuffer >= hardext.maxdrawbuffers) {
            errorShim(GL_INVALID_VALUE);
            return;
        }
        if (drawbuffer >= glstate->fbo.fbo_draw->n_draw) return; // GL_NONE...
        attch = glstate->fbo.fbo_draw->drawbuff[drawbuffer];
        if (attch == GL_NONE) return;
        if (is_default_fbo(glstate->fbo.fbo_draw)) {
            if (!(attch == GL_FRONT || attch == GL_BACK)) {
                errorShim(GL_INVALID_VALUE);
                return;
            }
        } else if (!(attch >= GL_COLOR_ATTACHMENT0 && attch < GL_COLOR_ATTACHMENT0 + hardext.maxdrawbuffers)) {
            errorShim(GL_INVALID_VALUE);
            return;
        }
        {
            ...
            LOAD_GLES_EXT(glDrawBuffers);
            // select the buffer...
            GLenum phys_attch = attch;
            if (!is_default_fbo(glstate->fbo.fbo_draw)) phys_attch = GL_COLOR_ATTACHMENT0 + drawbuffer;
            if (hardext.drawbuffers) gles_glDrawBuffers(1, &phys_attch);
            ...
            if (hardext.drawbuffers) {
                GLenum* restore = (GLenum*)alloca(glstate->fbo.fbo_draw->n_draw * sizeof(GLenum));
                remap_drawbuffers_for_fbo(glstate->fbo.fbo_draw, glstate->fbo.fbo_draw->n_draw,
                                          glstate->fbo.fbo_draw->drawbuff, restore, GL_FALSE);
                gles_glDrawBuffers(glstate->fbo.fbo_draw->n_draw, restore);
            }
            return;
        }
        break;
...
void APIENTRY_GL4ES gl4es_glClearNamedFramebufferiv(GLuint framebuffer, GLenum buffer, GLint drawbuffer,
                                                    const GLint* value) {
    GLuint oldf = glstate->fbo.fbo_draw->id;
...
void APIENTRY_GL4ES gl4es_glClearNamedFramebufferuiv(GLuint framebuffer, GLenum buffer, GLint drawbuffer,
                                                     const GLuint* value) {
    GLuint oldf = glstate->fbo.fbo_draw->id;
    GLenum target = (glstate->fbo.fbo_draw == glstate->fbo.fbo_read) ? GL_FRAMEBUFFER : GL_DRAW_FRAMEBUFFER;
    gl4es_glBindFramebuffer(target, framebuffer);
...
void APIENTRY_GL4ES gl4es_glClearNamedFramebufferfv(GLuint framebuffer, GLenum buffer, GLint drawbuffer,
                                                    const GLfloat* value) {
    GLuint oldf = glstate->fbo.fbo_draw->id;
    GLenum target = (glstate->fbo.fbo_draw == glstate->fbo.fbo_read) ? GL_FRAMEBUFFER : GL_DRAW_FRAMEBUFFER;
    gl4es_glBindFramebuffer(target, framebuffer);
...
void APIENTRY_GL4ES gl4es_glClearNamedFramebufferfi(GLuint framebuffer, GLenum buffer, GLint drawbuffer, GLfloat depth,
                                                    GLint stencil) {
    GLuint oldf = glstate->fbo.fbo_draw->id;
    GLenum target = (glstate->fbo.fbo_draw == glstate->fbo.fbo_read) ? GL_FRAMEBUFFER : GL_DRAW_FRAMEBUFFER;
    gl4es_glBindFramebuffer(target, framebuffer);
...
void APIENTRY_GL4ES gl4es_glColorMaskIndexed(GLuint framebuffer, GLboolean red, GLboolean green, GLboolean blue,
                                             GLboolean alpha) {
    GLuint oldf = glstate->fbo.fbo_draw->id;
    GLenum target = (glstate->fbo.fbo_draw == glstate->fbo.fbo_read) ? GL_FRAMEBUFFER : GL_DRAW_FRAMEBUFFER;
    gl4es_glBindFramebuffer(target, framebuffer);
...
AliasExport(void, glDeleteFramebuffers, , (GLsizei n, const GLuint* framebuffers));
...
AliasExport(void, glFramebufferTexture1D, ,
            (GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level));
AliasExport(void, glFramebufferTexture2D, ,
            (GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level));
AliasExport(void, glFramebufferTexture3D, ,
...
AliasExport(void, glFramebufferRenderbuffer, ,
            (GLenum target, GLenum attachment, GLenum renderbuffertarget, GLuint renderbuffer));
AliasExport(void, glDeleteRenderbuffers, , (GLsizei n, const GLuint* renderbuffers));
...
AliasExport(void, glDeleteFramebuffers, EXT, (GLsizei n, const GLuint* framebuffers));
...
AliasExport(void, glFramebufferTexture1D, EXT,
            (GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level));
AliasExport(void, glFramebufferTexture2D, EXT,
            (GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level));
AliasExport(void, glFramebufferTexture3D, EXT,
            (GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level, GLint layer));
AliasExport(void, glGenRenderbuffers, EXT, (GLsizei n, GLuint* renderbuffers));
AliasExport(void, glFramebufferRenderbuffer, EXT,
            (GLenum target, GLenum attachment, GLenum renderbuffertarget, GLuint renderbuffer));
AliasExport(void, glDeleteRenderbuffers, EXT, (GLsizei n, const GLuint* renderbuffers));
...
AliasExport(void, glGetFramebufferAttachmentParameteriv, EXT,
            (GLenum target, GLenum attachment, GLenum pname, GLint* params));
AliasExport(void, glGetRenderbufferParameteriv, EXT, (GLenum target, GLenum pname, GLint* params));
AliasExport(void, glFramebufferTextureLayer, EXT,
            (GLenum target, GLenum attachment, GLuint texture, GLint level, GLint layer));
AliasExport(void, glBlitFramebuffer, EXT,
            (GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1,
             GLbitfield mask, GLenum filter));

// Multisample stub
AliasExport(void, glRenderbufferStorageMultisample, ,
...
AliasExport(void, glClearNamedFramebufferiv, ,
            (GLuint framebuffer, GLenum buffer, GLint drawbuffer, const GLint* value));
AliasExport(void, glClearNamedFramebufferuiv, ,
            (GLuint framebuffer, GLenum buffer, GLint drawbuffer, const GLuint* value));
AliasExport(void, glClearNamedFramebufferfv, ,
            (GLuint framebuffer, GLenum buffer, GLint drawbuffer, const GLfloat* value));
AliasExport(void, glClearNamedFramebufferfi, ,
            (GLuint framebuffer, GLenum buffer, GLint drawbuffer, GLfloat depth, GLint stencil));

AliasExport(void, glClearNamedFramebufferiv, EXT,
            (GLuint framebuffer, GLenum buffer, GLint drawbuffer, const GLint* value));
AliasExport(void, glClearNamedFramebufferuiv, EXT,
            (GLuint framebuffer, GLenum buffer, GLint drawbuffer, const GLuint* value));
AliasExport(void, glClearNamedFramebufferfv, EXT,
            (GLuint framebuffer, GLenum buffer, GLint drawbuffer, const GLfloat* value));

...
#else
__declspec(dllimport) void __stdcall GetSystemTimeAsFileTime(unsigned __int64*);
...
void APIENTRY_GL4ES gl4es_glInterleavedArrays(GLenum format, GLsizei stride, const GLvoid* pointer) {
    ...
    if (!stride) stride = tex * gl_sizeof(tf) + color * gl_sizeof(cf) + normal * gl_sizeof(nf) + vert * gl_sizeof(vf);
    if (tex) {
...
void APIENTRY_GL4ES gl4es_glEnd(void) {
    ...
    for (int a = 0; a < hardext.maxtex; a++)
        if ((hardext.esversion == 1) && glstate->enable.texture[a] &&
            ...
            rlMultiTexCoord4f(glstate->list.active, GL_TEXTURE0 + a, glstate->texcoord[a][0], glstate->texcoord[a][1],
                              glstate->texcoord[a][2], glstate->texcoord[a][3]);
    ...
    if (withColor) gl4es_glColor4f(glstate->color[0], glstate->color[1], glstate->color[2], glstate->color[3]);
    noerrorShim();
}
...
void APIENTRY_GL4ES gl4es_glColor4fv(GLfloat* v) {
    if (glstate->list.active) {
        if (glstate->list.active->stage != STAGE_DRAW) {
            ...
            if (!glstate->list.pending) return gl4es_glColor4f(v[0], v[1], v[2], v[3]);
        } else {
...
void APIENTRY_GL4ES gl4es_glTexCoord4f(GLfloat s, GLfloat t, GLfloat r, GLfloat q) {
    if (glstate->list.active) {
        if (glstate->list.pending)
            ...
            // test if called between glBegin / glEnd but Texture is not active and not using a program. In that case,
            // ignore the call
    ...
    if (glstate->list.active) {
        if (glstate->list.pending)
            ...
            if (hardext.esversion == 1 ||
                (glstate->list.begin && (glstate->list.compiling || glstate->enable.texture[target - GL_TEXTURE0])))
...
void ToBuffer(int first, int count) {
    // this is hacky. Only the fpe VA should be treated here (but then, the consistancy check is a bit more difficult to
    // do)
    if (count < 13) return; // no VBO for smallest ones (4 triangles)
    glstate->vao->locked = globals4es.usevbo;
    // Strategy: compile only VA that are interleaved. So only 1 "Buffer" is compiled. Out of the buffer VA are not
    // compiled That should works with Quake3 engine that expect only Vertices array to be Compiled, but still allow to
    // build more complex arrays
    for (int i = 0; i < NB_VA; i++)
        if (glstate->vao->vertexattrib[i].enabled &&
            (!valid_vertex_type(glstate->vao->vertexattrib[i].type) || glstate->vao->vertexattrib[i].real_buffer)) {
            return;
        }
    ...
    if (!stride)
        stride = gl_sizeof(glstate->vao->vertexattrib[ATT_VERTEX].type) * glstate->vao->vertexattrib[ATT_VERTEX].size;
...
// move "master" data if there
    ...
    // glstate->vao->vertexattrib[ATT_VERTEX].real_pointer, PrintEnum(glstate->vao->vertexattrib[ATT_VERTEX].type),
    // glstate->vao->vertexattrib[ATT_VERTEX].size, glstate->vao->vertexattrib[ATT_VERTEX].stride);
    //  unbind the buffer
...
static renderlist_t* gl4es_glGetList(GLuint list) {
    ...
    if (k != kh_end(lists)) return kh_value(lists, k);
    return NULL;
}
...
#define call(name, type)                                                                                               \
    case name:                                                                                                         \
        gl4es_glCallList(((type*)lists)[i] + glstate->list.base);                                                      \
        break
...
GLboolean APIENTRY_GL4ES gl4es_glIsList(GLuint list) {
    ...
    khash_t(gllisthead)* lists = glstate->headlists;
    k = kh_get(gllisthead, lists, list);
    if (k != kh_end(lists)) return GL_TRUE;
    return GL_FALSE;
}
...
void APIENTRY_GL4ES gl4es_glPolygonMode(GLenum face, GLenum mode) {
    ERROR_IN_BEGIN
    noerrorShimNoPurge();
    if (face == GL_FRONT) face = GL_FRONT_AND_BACK; // TODO, better handle all this
    if (face == GL_BACK) return;                    // TODO, handle face enum for polygon mode != GL_FILL
    if (glstate->list.active)
        if (glstate->list.compiling) {
...
#ifndef NOX11
    if (glstate->emulatedPixmap && !glstate->emulatedWin) BlitEmulatedPixmap(0);
#endif
}
...
void APIENTRY_GL4ES gl4es_glShadeModel(GLenum mode) {
    ...
    noerrorShim();
    if (mode == glstate->shademodel) return;
...
void APIENTRY_GL4ES gl4es_glAlphaFunc(GLenum func, GLclampf ref) {
    ...
    if (glstate->alphafunc == func && glstate->alpharef == ref) return;
    if (func != GL_NEVER && func != GL_LESS && func != GL_EQUAL && func != GL_LEQUAL && func != GL_GREATER &&
        func != GL_NOTEQUAL && func != GL_ALWAYS && func != GL_GEQUAL) {
        errorShim(GL_INVALID_ENUM);
        return;
    }
...
void gl4es_glVertexP2ui(GLuint p1, GLuint p2) __attribute__((visibility("default"))) {
    if (glstate->list.active) {
        ...
        glstate->vertex[0] = (GLfloat)p1;
        glstate->vertex[1] = (GLfloat)p2;
    }
}
...
void gl4es_glVertexP2uiv(GLuint* p) __attribute__((visibility("default"))) {
    if (glstate->list.active) {
        ...
        glstate->vertex[0] = (GLfloat)p[0];
        glstate->vertex[1] = (GLfloat)p[1];
    }
}

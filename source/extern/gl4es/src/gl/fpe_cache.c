...
static kh_inline khint_t _hash_fpe(fpe_state_t* p) {
    ...
    khint_t h = (khint_t)*s;
    for (int i = 1; i < sizeof(fpe_state_t); ++i)
        h = (h << 5) - h + (khint_t) * (++s);
    return h;
}
...
#define KHASH_MAP_INIT_FPE(name, khval_t) KHASH_INIT(name, kh_fpe_t, khval_t, 1, kh_fpe_hash_func, kh_fpe_hash_equal)
...
void fpe_disposeCache(fpe_cache_t* cache, int freeprog) {
    ...
    kh_foreach_value(
        cache, m, if (freeprog) {
            if (m->glprogram) gl4es_glDeleteProgram(m->glprogram->id);
        } free(m);) kh_destroy(fpecachelist, cache);
}
...
void fpe_readPSA() {
    ...
    fclose(f);
    DBG(SHUT_LOGD("Loaded a PSA with %d Precompiled Programs\n", psa->size);)
}

void fpe_writePSA() {
    if (!psa || !psa_name) return;
    if (!psa->dirty) return; // no need
    ...
    fclose(f);
    DBG(SHUT_LOGD("Saved a PSA with %d Precompiled Programs\n", psa->size);)
}

void fpe_InitPSA(const char* name) {
    if (psa) return; // already inited
...
void fpe_FreePSA() {
    if (!psa) return; // nothing to init

    psa_t* m;
    kh_foreach_value(psa->cache, m, free(m->prog); free(m);) kh_destroy(psalist, psa->cache);
...
int fpe_GetProgramPSA(GLuint program, fpe_state_t* state) {
    ...
    if (state->vertex_prg_enable || state->fragment_prg_enable) return 0;
    khint_t k = kh_get(psalist, psa->cache, state);
    if (k == kh_end(psa->cache)) return 0; // not here
...
void fpe_AddProgramPSA(GLuint program, fpe_state_t* state) {
    if (!psa) return;
    // if state contains custom vertex of fragment shader, then ignore
    if (state->vertex_prg_enable || state->fragment_prg_enable) return;
    psa->dirty = 1;
    psa_t* p = (psa_t*)calloc(1, sizeof(psa_t));

...
#define map(fmt, ...)                                                                                                  \
    ...
    default:
        DBGLOGD("LIBGL: unknown pixel format %s\n", PrintEnum(format));
        break;
    }
...
static inline bool remap_pixel(const GLvoid* src, GLvoid* dst, const colorlayout_t* src_color, GLenum src_type,
...
#define default(arr, amod, vmod, hmod, key, def) (key >= 0) ? hmod(arr[amod key] vmod) : def
...
#define write_each(amod, vmod, hmod)                                                                                   \
    carefully(d, amod, hmod, dst_color->red, pixel.r vmod) carefully(d, amod, hmod, dst_color->green, pixel.g vmod)    \
    ...
    switch (src_type) {
    type_case(GL_UNSIGNED_INT, GLuint, read_each(, / 4294967295.0f, )) type_case(GL_DOUBLE, GLdouble, read_each(, , ))
        type_case(GL_FLOAT, GLfloat, read_each(, , ))
            type_case(GL_HALF_FLOAT_OES, halffloat_t, read_each(, , float_h2f))
                type_case(GL_BYTE, GLbyte, read_each(, / 128.0f, )) case GL_INT8_REV:
        ...
        default:
            // TODO: add glSetError?
            DBGLOGD("LIBGL: Unsupported source data type: %s\n", PrintEnum(src_type));
        return false;
        break;
    }
    ...
    switch (dst_type) {
        type_case(GL_UNSIGNED_INT, GLuint, read_each(, / 4294967295.0f, ))
            ...
            type_case(GL_UNSIGNED_SHORT_5_6_5, GLushort, GLfloat color[4]; color[dst_color->red] = pixel.r;
                      color[dst_color->green] = pixel.g; color[dst_color->blue] = pixel.b;
                      *d = (((GLuint)(color[0] * 31.0f) & 0x1f) << 11) | (((GLuint)(color[1] * 63.0f) & 0x3f) << 5) |
                           (((GLuint)(color[2] * 31.0f) & 0x1f));)
                type_case(GL_UNSIGNED_SHORT_5_5_5_1, GLushort, GLfloat color[4]; color[dst_color->red] = pixel.r;
                          color[dst_color->green] = pixel.g; color[dst_color->blue] = pixel.b;
                          color[dst_color->alpha] = pixel.a;
                              // TODO: can I macro this or something? it follows a pretty strict form.
                                  * d = (((GLuint)(color[3]) & 0x01)) | (((GLuint)(color[2] * 31.0f) & 0x1f) << 1) |
                                        (((GLuint)(color[1] * 31.0f) & 0x1f) << 6) |
                                        (((GLuint)(color[0] * 31.0f) & 0x1f) << 11);)
                    type_case(GL_UNSIGNED_SHORT_1_5_5_5_REV, GLushort, GLfloat color[4];
                              color[dst_color->red] = pixel.r; color[dst_color->green] = pixel.g;
                              color[dst_color->blue] = pixel.b; color[dst_color->alpha] = pixel.a;
                              *d = (((GLuint)(color[3]) & 0x01) << 15) | (((GLuint)(color[2] * 31.0f) & 0x1f) << 10) |
                                   (((GLuint)(color[1] * 31.0f) & 0x1f) << 5) | (((GLuint)(color[0] * 31.0f) & 0x1f));)
                        type_case(GL_UNSIGNED_SHORT_4_4_4_4, GLushort, GLfloat color[4];
                                  color[dst_color->red] = pixel.r; color[dst_color->green] = pixel.g;
                                  color[dst_color->blue] = pixel.b; color[dst_color->alpha] = pixel.a;
                                  *d = (((GLushort)(color[0] * 15.0f) & 0x0f) << 12) |
                                       (((GLushort)(color[1] * 15.0f) & 0x0f) << 8) |
                                       (((GLushort)(color[2] * 15.0f) & 0x0f) << 4) |
                                       (((GLushort)(color[3] * 15.0f) & 0x0f));) default
            : DBGLOGD("LIBGL: Unsupported target data type: %s\n", PrintEnum(dst_type));
        return false;
        break;
    }
    return true;
...
static inline bool transform_pixel(const GLvoid* src, GLvoid* dst, const colorlayout_t* src_color, GLenum src_type,
...
#define default(arr, amod, vmod, hmod, key, def) (amod key) >= 0 ? hmod(arr[amod key] vmod) : def
...
#define write_each(amod, vmod, hmod)                                                                                   \
    carefully(d, amod, hmod, src_color->red, pixel.r vmod) carefully(d, amod, hmod, src_color->green, pixel.g vmod)    \
...
#define transformf(pix, number)                                                                                        \
    ...
    type_case(GL_UNSIGNED_INT, GLuint, read_each(, / 4294967295.0f, )) type_case(GL_DOUBLE, GLdouble, read_each(, , ))
        type_case(GL_FLOAT, GLfloat, read_each(, , ))
            type_case(GL_HALF_FLOAT_OES, halffloat_t, read_each(, , float_h2f)) case GL_INT8_REV:
        type_case(GL_UNSIGNED_BYTE, GLubyte, read_each(, / 255.0f, ))
            type_case(GL_UNSIGNED_SHORT, GLushort, read_each(, / 65535.0f, ))
                type_case(GL_INT8, GLubyte, read_each(max_a -, / 255.0f, ))
                    type_case(GL_UNSIGNED_SHORT_5_6_5, GLushort,
                              ...
                              read_each(, / 63.0f, );) type_case(GL_UNSIGNED_SHORT_5_5_5_1, GLushort,
                                                                 s =
                                                                     (const GLushort[]){
                                                                         ((v >> 11) & 0x1f),
                                                                         ((v >> 6) & 0x1f),
                                                                 ...
                                                                 read_each(, / 31.0f, );)
                        type_case(GL_UNSIGNED_SHORT_4_4_4_4, GLushort,
                                  s = (const GLushort[]){((v >> 12) & 0x0f), ((v >> 8) & 0x0f), ((v >> 4) & 0x0f),
                                                         ((v) & 0x0f)};
                                  read_each(, / 15.0f, );) default :
            // TODO: add glSetError?
            DBGLOGD("LIBGL: transform_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
        return false;
        break;
    }
                    ...
                    type_case(GL_UNSIGNED_BYTE, GLubyte, write_each(, *255.0, ))
                        type_case(GL_UNSIGNED_SHORT, GLushort, write_each(, / 65535.0f, ))
                            type_case(GL_INT8_REV, GLubyte, write_each(, *255.0, ))
                                type_case(GL_INT8, GLubyte, write_each(max_a -, *255.0, ))
            // TODO: force 565 to RGB? then we can change [4] -> 3
            type_case(GL_UNSIGNED_SHORT_5_6_5, GLushort, GLfloat color[4]; color[src_color->red] = pixel.r;
                      color[src_color->green] = pixel.g; color[src_color->blue] = pixel.b;
                      *d = (((GLuint)(color[0] * 31.0f) & 0x1f) << 11) | (((GLuint)(color[1] * 63.0f) & 0x3f) << 5) |
                           (((GLuint)(color[2] * 31.0f) & 0x1f));)
                type_case(GL_UNSIGNED_SHORT_5_5_5_1, GLushort, GLfloat color[4]; color[src_color->red] = pixel.r;
                          color[src_color->green] = pixel.g; color[src_color->blue] = pixel.b;
                          color[src_color->alpha] = pixel.a;
                              // TODO: can I macro this or something? it follows a pretty strict form.
                                  * d = (((GLuint)(color[3]) & 0x01)) | (((GLuint)(color[2] * 31.0f) & 0x1f) << 1) |
                                        (((GLuint)(color[1] * 31.0f) & 0x1f) << 6) |
                                        (((GLuint)(color[0] * 31.0f) & 0x1f) << 11);)
                    type_case(GL_UNSIGNED_SHORT_4_4_4_4, GLushort, GLfloat color[4]; color[src_color->red] = pixel.r;
                              color[src_color->green] = pixel.g; color[src_color->blue] = pixel.b;
                              color[src_color->alpha] = pixel.a; *d = (((GLushort)(color[0] * 15.0f) & 0x0f) << 12) |
                                                                      (((GLushort)(color[1] * 15.0f) & 0x0f) << 8) |
                                                                      (((GLushort)(color[2] * 15.0f) & 0x0f) << 4) |
                                                                      (((GLushort)(color[3] * 15.0f) & 0x0f));) default
            : DBGLOGD("LIBGL: Unsupported target data type: %s\n", PrintEnum(src_type));
...
static inline bool half_pixel(const GLvoid* src0, const GLvoid* src1, const GLvoid* src2, const GLvoid* src3,
                              GLvoid* dst, const colorlayout_t* src_color, GLenum src_type) {
...
#define write_each(amod, vmod, hmod)                                                                                   \
    ...
    type_case(GL_UNSIGNED_INT, GLuint, read_each(, / 4294967295.0f, )) type_case(GL_DOUBLE, GLdouble, read_each(, , ))
        ...
        type_case(GL_UNSIGNED_BYTE, GLubyte, read_each(, / 255.0f, ))
            type_case(GL_UNSIGNED_SHORT, GLushort, read_each(, / 65535.0f, ))
                type_case(GL_INT8, GLubyte, read_each(max_a -, / 255.0f, )) type_case(
                    ...
                    type_case(
                        ...
                        type_case(
                            ...
                            for (int ii = 0; ii < 4; ii++) {
                                s[ii] = (const GLushort[]){((v[ii] >> 12) & 0x0f), ((v[ii] >> 8) & 0x0f),
                                                           ((v[ii] >> 4) & 0x0f), ((v[ii]) & 0x0f)};
                                read_i_each(, / 15.0f, , ii);
                            };) default :
            // TODO: add glSetError?
            DBGLOGD("LIBGL: half_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
        return false;
        break;
    }
                ...
                type_case(GL_UNSIGNED_SHORT_5_5_5_1, GLushort, GLfloat color[4]; color[src_color->red] = pixel.r;
                    ...
                    type_case(GL_UNSIGNED_SHORT_4_4_4_4, GLushort, GLfloat color[4]; color[src_color->red] = pixel.r;
                              ...
                              color[src_color->alpha] = pixel.a; *d = (((GLushort)(color[0] * 15.0f) & 0x0f) << 12) |
                                                                      ...
                                                                      (((GLushort)(color[2] * 15.0f) & 0x0f) << 4) |
                                                                      (((GLushort)(color[3] * 15.0f) & 0x0f));) default
            : DBGLOGD("LIBGL: half_pixel: Unsupported target data type: %s\n", PrintEnum(src_type));
        return false;
        break;
    }
    return true;
...
static inline bool quarter_pixel(const GLvoid* src[16], GLvoid* dst, const colorlayout_t* src_color, GLenum src_type) {
                    ...
                    type_case(
                        ...
                        type_case(
                            ...
                            for (int ii = 0; ii < 16; ii++) {
                                s[ii] = (GLushort[]){((v[ii] & 0xf000) >> 12), ((v[ii] & 0x0f00) >> 8),
                                                     ((v[ii] & 0x00f0) >> 4), (v[ii] & 0x000f)};
                                read_i_each(, / 15.0f, , ii);
                            };) default :
            // TODO: add glSetError?
            DBGLOGD("LIBGL: quarter_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
        return false;
        break;
    }
    pixel.r = (pix[0].r + pix[1].r + pix[2].r + pix[3].r + pix[4].r + pix[5].r + pix[6].r + pix[7].r + pix[8].r +
               pix[9].r + pix[10].r + pix[11].r + pix[12].r + pix[13].r + pix[14].r + pix[15].r) *
              0.0625f;
    pixel.g = (pix[0].g + pix[1].g + pix[2].g + pix[3].g + pix[4].g + pix[5].g + pix[6].g + pix[7].g + pix[8].g +
               pix[9].g + pix[10].g + pix[11].g + pix[12].g + pix[13].g + pix[14].g + pix[15].g) *
              0.0625f;
    pixel.b = (pix[0].b + pix[1].b + pix[2].b + pix[3].b + pix[4].b + pix[5].b + pix[6].b + pix[7].b + pix[8].b +
               pix[9].b + pix[10].b + pix[11].b + pix[12].b + pix[13].b + pix[14].b + pix[15].b) *
              0.0625f;
    pixel.a = (pix[0].a + pix[1].a + pix[2].a + pix[3].a + pix[4].a + pix[5].a + pix[6].a + pix[7].a + pix[8].a +
               pix[9].a + pix[10].a + pix[11].a + pix[12].a + pix[13].a + pix[14].a + pix[15].a) *
              0.0625f;
                ...
                type_case(GL_UNSIGNED_SHORT_5_5_5_1, GLushort, GLfloat color[4]; color[src_color->red] = pixel.r;
                    ...
                    type_case(GL_UNSIGNED_SHORT_4_4_4_4, GLushort, GLfloat color[4]; color[src_color->red] = pixel.r;
                              ...
                              color[src_color->alpha] = pixel.a; *d = (((GLushort)(color[0] * 15.0f) & 0x0f) << 12) |
                                                                      ...
                                                                      (((GLushort)(color[2] * 15.0f) & 0x0f) << 4) |
                                                                      (((GLushort)(color[3] * 15.0f) & 0x0f));) default
            : DBGLOGD("LIBGL: quarter_pixel Unsupported target data type: %s\n", PrintEnum(src_type));
        return false;
        break;
    }
    return true;
...
bool pixel_convert(const GLvoid* src, GLvoid** dst, GLuint width, GLuint height, GLenum src_format, GLenum src_type,
    ...
    if (!remap_pixel((const GLvoid*)src_pos, (GLvoid*)dst_pos, src_color, src_type, dst_color, dst_type)) {
        // fake convert, to get if it's ok or not
        return false;
    }
    // special case for GL_COLOR_INDEX
    if (src_format == GL_COLOR_INDEX) {
        if (src_type != GL_UNSIGNED_BYTE) return false; // only unsigned byte for now
        GLubyte tmp[4];
        int idx;
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                remap_pixel((const GLvoid*)tmp, (GLvoid*)dst_pos, src_color, GL_FLOAT, dst_color, dst_type);
                src_pos += src_stride;
                dst_pos += dst_stride;
            }
            dst_pos += dst_width;
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                remap_pixel((const GLvoid*)src_pos, (GLvoid*)dst_pos, src_color, src_type, dst_color, dst_type);
                src_pos += src_stride;
                dst_pos += dst_stride;
            }
            dst_pos += dst_width;
...
bool pixel_transform(const GLvoid* src, GLvoid** dst, GLuint width, GLuint height, GLenum src_format, GLenum src_type,
    ...
    src_color = get_color_map(src_format);
    GLsizei src_stride = pixel_sizeof(src_format, src_type);
    if (*dst == src || *dst == NULL) *dst = malloc(dst_size);
    uintptr_t src_pos = (uintptr_t)src;
    uintptr_t dst_pos = (uintptr_t)*dst;
    if (!transform_pixel((const GLvoid*)src_pos, (GLvoid*)dst_pos, src_color, src_type, scales, bias)) {
        // fake convert, to get if it's ok or not
        return false;
    }
    for (int aa = 0; aa < dst_size; aa++) {
        for (int i = 0; i < pixels; i++) {
            transform_pixel((const GLvoid*)src_pos, (GLvoid*)dst_pos, src_color, src_type, scales, bias);
            src_pos += src_stride;
            dst_pos += src_stride;
        }
        return true;
...
bool pixel_scale(const GLvoid* old, GLvoid** new, GLuint width, GLuint height, GLuint new_width, GLuint new_height,
    ...
    for (int y = 0; y < new_height; y++) {
        ...
        for (int x = 0; x < new_width; x++) {
            ...
            pixel = src + (oldx + oldy * width) * pixel_size;
...
bool pixel_halfscale(const GLvoid* old, GLvoid** new, GLuint width, GLuint height, GLenum format, GLenum type) {
    if (!old) {
        *new = NULL;
        return true;
    }
...
bool pixel_thirdscale(const GLvoid* old, GLvoid** new, GLuint width, GLuint height, GLenum format, GLenum type) {
    ...
    //    DBGLOGD("LIBGL: halfscaling %ux%u -> %ux%u\n", width, height, new_width, new_height);
    const colorlayout_t* src_color;
    ...
    for (int y = 0; y < new_height; y++) {
        for (int x = 0; x < new_width; x++) {
            pix0 = src + ((x * mx) + (y * my) * width) * pixel_size;
            pix1 = src + ((x * mx + dx) + (y * my) * width) * pixel_size;
            pix2 = src + ((x * mx) + (y * my + dy) * width) * pixel_size;
            pix3 = src + ((x * mx + dx) + (y * my + dy) * width) * pixel_size;
            half_pixel((GLvoid*)pix0, (GLvoid*)pix1, (GLvoid*)pix2, (GLvoid*)pix3, (GLvoid*)tmp, src_color, type);
            *((GLushort*)pos) = (((GLushort)tmp[0]) & 0xf0) << 8 | (((GLushort)tmp[1]) & 0xf0) << 4 |
...
bool pixel_quarterscale(const GLvoid* old, GLvoid** new, GLuint width, GLuint height, GLenum format, GLenum type) {
    GLuint pixel_size, new_width, new_height;
    new_width = width / 4;
    if (!new_width) ++new_width;
    new_height = height / 4;
    ...
    /*    if (new_width*4!=width || new_height*4!=height) {
            DBGLOGD("LIBGL: quarterscaling %ux%u failed\n", width, height);
            return false;
        }*/
    //    DBGLOGD("LIBGL: quarterscaling %ux%u -> %ux%u\n", width, height, new_width, new_height);
    const colorlayout_t* src_color;
    src_color = get_color_map(format);
    GLvoid* dst;
    ...
    if (!src_color->type) {
        if (!pixel_size) {
            DBGLOGD("LIBGL: Cannot quarterscale unknown format/type %s/%s\n", PrintEnum(format), PrintEnum(type));
            free(dst);
            return 0;
        }
        for (int y = 0; y < new_height; y++) {
            for (int x = 0; x < new_width; x++) {
                pix[0] = src + ((x * 4) + (y * 4) * width) * pixel_size;
        ...
        for (int x = 0; x < new_width; x++) {
            for (int dx = 0; dx < 4; dx++) {
                for (int dy = 0; dy < 4; dy++) {
                    pix[dx + dy * 4] = src + ((x * 4 + dxs[dx]) + (y * 4 + dys[dy]) * width) * pixel_size;
                }
            }
...
bool pixel_doublescale(const GLvoid* old, GLvoid** new, GLuint width, GLuint height, GLenum format, GLenum type) {
    if (!old) {
        *new = NULL;
        return true;
    }

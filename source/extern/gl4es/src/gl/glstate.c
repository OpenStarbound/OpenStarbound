...
static void free_renderbuffer(glrenderbuffer_t* rend) {
    ...
    if (!rend || !gles_glDeleteRenderbuffers) return;
    if (rend->secondarybuffer) gles_glDeleteRenderbuffers(1, &rend->secondarybuffer);
    if (rend->renderbuffer) gles_glDeleteRenderbuffers(1, &rend->renderbuffer);
...
static void free_framebuffer(glframebuffer_t* fb) {
    ...
    if (!fb || !gles_glDeleteFramebuffers) return;
    if (fb->id) gles_glDeleteFramebuffers(1, &fb->id);
    // the texture will be free by the free of the texture list, as it's referenced there...
    free(fb);
}
...
static void free_texture(gltexture_t* tex) {
    ...
    if (!tex || !gles_glDeleteTextures) return;
    if (tex->glname) gles_glDeleteTextures(1, &tex->glname);
...
void* NewGLState(void* shared_glstate, int es2only) {
    glstate_t* glstate =
        (shared_glstate != DEFAULT_STATE) ? ((glstate_t*)calloc(1, sizeof(glstate_t))) : &default_glstate;
#if defined(AMIGAOS4) || defined(__EMSCRIPTEN__)
    int def = 0;
    if (shared_glstate == DEFAULT_STATE) def = 1;
#endif
    if (shared_glstate == DEFAULT_STATE) shared_glstate = NULL;
    if (shared_glstate) {
    ...
    */
    ...
    glstate->light.lights[0].diffuse[0] = glstate->light.lights[0].diffuse[1] = glstate->light.lights[0].diffuse[2] =
    ...
    // Materials
    glstate->material.front.ambient[0] = glstate->material.front.ambient[1] = glstate->material.front.ambient[2] = 0.2f;
    glstate->material.front.ambient[3] = 1.0f;
    glstate->material.front.diffuse[0] = glstate->material.front.diffuse[1] = glstate->material.front.diffuse[2] = 0.8f;
    ...
    for (int i = 0; i < hardext.maxtex; i++) {
        ...
        glstate->texenv[i].env.op2_rgb = glstate->texenv[i].env.op0_alpha = glstate->texenv[i].env.op1_alpha =
            glstate->texenv[i].env.op2_alpha = GL_SRC_ALPHA;
    }
    // TexGen
    for (int i = 0; i < hardext.maxtex; i++) {
    ...
    if (hardext.esversion > 1) {
        glstate->fpe_state = (fpe_state_t*)calloc(1, sizeof(fpe_state_t));
        glstate->glsl->es2 = es2only;
        if (!shared_glstate) fpe_Init(glstate);
...
void DeleteGLState(void* oldstate) {
    ...
    if (oldstate == DEFAULT_STATE) state = &default_glstate;
    ...
    if (globals4es.noclean) return;

    if (glstate == state) glstate = (oldstate == DEFAULT_STATE) ? NULL : &default_glstate;

    if (!state->shared_cnt) free(state->actual_tex2d);

#define free_hashmap(T, N, K, F)                                                                                       \
    if (state->N) {                                                                                                    \
        T* m;                                                                                                          \
        kh_foreach_value(state->N, m, F(m);) kh_destroy(K, state->N);                                                  \
    }
...
#undef free_matrix
    ...
    if (state->clientStack) free(state->clientStack);
    // linestipple
    if (state->linestipple.data) free(state->linestipple.data);
    // raster / bitmap
    if (state->raster.data) free(state->raster.data);
    if (state->raster.bitmap) free(state->raster.bitmap);
    ...
    for (int a = 0; a < MAX_TEX - 2; ++a)
        if (state->merger_tex[a]) free(state->merger_tex[a]);
    // mainfbo
    if (!state->shared_cnt) {
        if (state->fbo.mainfbo_fbo) deleteMainFBO(state);
    }
    // oldfbos
    ...
    // get extensions
    if (state->extensions) free(state->extensions);
    ...
    for (int i = 0; i < MAX_TEX; ++i) {
        if (state->helper_tex[i]) free(state->helper_tex[i]);
        if (state->texgened[i]) free(state->texgened[i]);
    }
    ...
    // all done
    if (oldstate != DEFAULT_STATE) free(state);
    return;
}

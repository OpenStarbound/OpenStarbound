...
namespace LuaBindings {
  ...
  LuaCallbacks makeWorldThreadCallbacks(World* world, ConnectionId originConnection = ServerConnectionId);
  LuaCallbacks makeWorldCallbacks(World* world, ConnectionId originConnection = ServerConnectionId);

  void addWorldDebugCallbacks(LuaCallbacks& callbacks);
  void addWorldEntityCallbacks(LuaCallbacks& callbacks, World* world, ConnectionId originConnection = ServerConnectionId);
  void addWorldEnvironmentCallbacks(LuaCallbacks& callbacks, World* world, ConnectionId originConnection = ServerConnectionId);
  ...
  namespace WorldCallbacks {
    bool placeObject(World* world, ConnectionId originConnection, String const& arg1, Vec2I const& arg2, Maybe<int> const& arg3, Json const& arg4);
    Maybe<EntityId> spawnItem(World* world, ConnectionId originConnection, Json const& itemType, Vec2F const& worldPosition, Maybe<size_t> const& inputCount, Json const& inputParameters, Maybe<Vec2F> const& initialVelocity, Maybe<float> const& intangibleTime);
    List<EntityId> spawnTreasure(World* world, ConnectionId originConnection, Vec2F const& position, String const& pool, float level, Maybe<uint64_t> seed);
    Maybe<EntityId> spawnMonster(World* world, ConnectionId originConnection, String const& arg1, Vec2F const& arg2, Maybe<JsonObject> const& arg3);
    Maybe<EntityId> spawnNpc(World* world, ConnectionId originConnection, Vec2F const& arg1, String const& arg2, String const& arg3, float arg4, Maybe<uint64_t> arg5, Json const& arg6);
    Maybe<EntityId> spawnStagehand(World* world, ConnectionId originConnection, Vec2F const& spawnPosition, String const& typeName, Json const& overrides);
    Maybe<EntityId> spawnProjectile(World* world, ConnectionId originConnection, String const& arg1, Vec2F const& arg2, Maybe<EntityId> const& arg3, Maybe<Vec2F> const& arg4, bool arg5, Json const& arg6);
    Maybe<EntityId> spawnVehicle(World* world, ConnectionId originConnection, String const& vehicleName, Vec2F const& pos, Json const& extraConfig);
    Json getProperty(World* world, String const& arg1, Json const& arg2);
    void setProperty(World* world, ConnectionId originConnection, String const& arg1, Json const& arg2);
  ...
  namespace ServerWorldCallbacks {
    ...
    bool breakObject(WorldServer* world, ConnectionId originConnection, EntityId arg1, bool arg2);
    ...
    void setTileProtection(WorldServer* world, ConnectionId originConnection, DungeonId arg1, bool arg2);
    bool isPlayerModified(WorldServer* world, RectI const& region);
    Maybe<LiquidLevel> forceDestroyLiquid(WorldServer* world, ConnectionId originConnection, Vec2F const& position);
    EntityId loadUniqueEntity(WorldServer* world, String const& uniqueId);
    void setUniqueId(WorldServer* world, ConnectionId originConnection, EntityId entityId, Maybe<String> const& uniqueId);
    Json takeItemDrop(World* world, EntityId entityId, Maybe<EntityId> const& takenBy);
    void setPlayerStart(WorldServer* world, ConnectionId originConnection, Vec2F const& playerStart, Maybe<bool> respawnInWorld);
  ...
  namespace WorldEntityCallbacks {
    ...
    bool containerClose(World* world, ConnectionId originConnection, EntityId entityId);
    bool containerOpen(World* world, ConnectionId originConnection, EntityId entityId);
    Json containerItems(World* world, EntityId entityId);
    Json containerItemAt(World* world, EntityId entityId, size_t offset);
    Maybe<bool> containerConsume(World* world, ConnectionId originConnection, EntityId entityId, Json const& items);
    Maybe<bool> containerConsumeAt(World* world, ConnectionId originConnection, EntityId entityId, size_t offset, int count);
    Maybe<size_t> containerAvailable(World* world, EntityId entityId, Json const& items);
    Json containerTakeAll(World* world, ConnectionId originConnection, EntityId entityId);
    Json containerTakeAt(World* world, ConnectionId originConnection, EntityId entityId, size_t offset);
    Json containerTakeNumItemsAt(World* world, ConnectionId originConnection, EntityId entityId, size_t offset, int const& count);
    ...
    Json containerAddItems(World* world, ConnectionId originConnection, EntityId entityId, Json const& items);
    Json containerStackItems(World* world, ConnectionId originConnection, EntityId entityId, Json const& items);
    Json containerPutItemsAt(World* world, ConnectionId originConnection, EntityId entityId, Json const& items, size_t offset);
    Json containerSwapItems(World* world, ConnectionId originConnection, EntityId entityId, Json const& items, size_t offset);
    Json containerSwapItemsNoCombine(World* world, ConnectionId originConnection, EntityId entityId, Json const& items, size_t offset);
    Json containerItemApply(World* world, ConnectionId originConnection, EntityId entityId, Json const& items, size_t offset);
    Maybe<LuaValue> callScriptedEntity(World* world, ConnectionId originConnection, EntityId entityId, String const& function, LuaVariadic<LuaValue> const& args);
  ...
  namespace WorldEnvironmentCallbacks {
    ...
    void setMaterialColor(World* world, ConnectionId originConnection, Vec2F const& position, String const& layerName, MaterialColorVariant color);
    bool damageTiles(World* world, ConnectionId originConnection, List<Vec2I> const& arg1, String const& arg2, Vec2F const& arg3, String const& arg4, float arg5, Maybe<unsigned> const& arg6, Maybe<EntityId> sourceEntity);
    bool damageTileArea(World* world, ConnectionId originConnection, Vec2F center, float radius, String layer, Vec2F sourcePosition, String damageType, float damage, Maybe<unsigned> const& harvestLevel, Maybe<EntityId> sourceEntity);
    bool placeMaterial(World* world, ConnectionId originConnection, Vec2I const& arg1, String const& arg2, String const& arg3, Maybe<int> const& arg4, bool arg5);
    bool replaceMaterials(World* world, ConnectionId originConnection, List<Vec2I> const& tilePositions, String const& layer, String const& materialName, Maybe<int> const& hueShift, bool enableDrops);
    bool replaceMaterialArea(World* world, ConnectionId originConnection, Vec2F center, float radius, String const& layer, String const& materialName, Maybe<int> const& hueShift, bool enableDrops);
    bool placeMod(World* world, ConnectionId originConnection, Vec2I const& arg1, String const& arg2, String const& arg3, Maybe<int> const& arg4, bool arg5);
    ...
    bool spawnLiquid(World* world, ConnectionId originConnection, Vec2F const& arg1, LiquidId arg2, float arg3);
    Maybe<LiquidLevel> destroyLiquid(World* world, ConnectionId originConnection, Vec2F const& position);
    bool isTileProtected(World* world, ConnectionId originConnection, Vec2F const& position);
  }
}
